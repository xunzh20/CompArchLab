...
// ALU
    ...
    if (ALUK == 0){
        ALU_data = Low16bits(CURRENT_LATCHES.REGS[SR1] + SR2MUX_data);
    }else if (ALUK == 1){
        ALU_data = Low16bits(CURRENT_LATCHES.REGS[SR1] & SR2MUX_data);
    }else if (ALUK == 2){
        ALU_data = Low16bits(CURRENT_LATCHES.REGS[SR1] ^ SR2MUX_data);
...
// need to fix cycle memory: should not directly store into MDR, need to hold it first and wait for ldMDR
//                           also need to fix logic, right now does not support ldb
//
/* 
*/