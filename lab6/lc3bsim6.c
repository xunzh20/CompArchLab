...
int MEM_PCMUX, TARGET_PC, TRAP_PC, V_MEM_LD_CC, V_MEM_LD_REG;

/*signals generated by AGEX stage and needed by previous stages*/
...
void MEM_stage() {
  ...
  // }
  if(Get_DCACHE_RW(PS.MEM_CS)) {
    WE_1 = Get_DATA_SIZE(PS.MEM_CS) || (PS.MEM_ADDRESS & 0x01);
    WE_0 = Get_DATA_SIZE(PS.MEM_CS) || !(PS.MEM_ADDRESS & 0x01);
  }

  int write_word = PS.MEM_ALU_RESULT;
  if (!Get_DATA_SIZE(PS.MEM_CS) && (PS.MEM_ADDRESS & 0x01)) // byte 
  {
    write_word = ((write_word & 0xFF ) << 8) + (write_word & 0xFF); // duplicate lower 8 bits
  }
  ...
  if (V_DCACHE_EN){
    dcache_access(PS.MEM_ADDRESS, &read_word, write_word, &dcache_r, WE_0, WE_1 );
  }

  // if ((Get_DATA_SIZE(PS.MEM_CS) == 0) && (PS.MEM_ADDRESS & 0x0001 == 0)){
  //   read_word = signext(read_word & 0x00FF,7);
  // }else{
  //   read_word = signext((read_word & 0xFF00) >> 8, 7);
  // }
  ...
  if (PS.MEM_V){
    if (Get_UNCOND_OP(PS.MEM_CS)){
      ...
      if(((PS.MEM_IR & 0x0800) && (PS.MEM_CC & 0x04)) || ((PS.MEM_IR & 0x0400) && (PS.MEM_CC & 0x02)) || ((PS.MEM_IR & 0x0200) && (PS.MEM_CC & 0x01))) {
        MEM_PCMUX = 1;
      }
    }
  }
  ...
  NEW_PS.SR_DATA = Low16bits(read_word);
...
void AGEX_stage() {
  ...
  int ADDR1MUX_OUT, ADDR2MUX_OUT;

  ADDR1MUX_OUT = Get_ADDR1MUX(PS.AGEX_CS) ? PS.AGEX_SR1 : PS.AGEX_NPC;
  
  int ADDR2MUX_SELECT = Get_ADDR2MUX(PS.AGEX_CS);
  ADDR2MUX_OUT = 0;
  if (ADDR2MUX_SELECT == 1){
    ADDR2MUX_OUT = Low16bits(signext(PS.AGEX_IR & 0x003F, 5));
  }
  else if (ADDR2MUX_SELECT == 2){
    ADDR2MUX_OUT = Low16bits(signext(PS.AGEX_IR & 0x01FF, 8));
  }
  else if (ADDR2MUX_SELECT == 3){
    ADDR2MUX_OUT = Low16bits(signext(PS.AGEX_IR & 0x07FF, 10));
  }
  ...
  if (Get_ADDRESSMUX(PS.AGEX_CS)){
    ADDRESSMUX_OUT = Low16bits(ADDR1MUX_OUT + ADDR2MUX_OUT);
  }
  else {
    ADDRESSMUX_OUT = Low16bits((PS.AGEX_IR & 0x00FF) << 1);
  }
  //shifter
  
  int SHIFTER_OUT = 0;
  // int SHIFT_AMOUNT = PS.AGEX_IR & 0x000F;
  // //right shift
  // if (PS.AGEX_IR & 0x0010){
  //   //RSHFA
  //   if (PS.AGEX_IR & 0x0020){
  //     SHIFTER_OUT = Low16bits(signext(PS.AGEX_SR1,15)>> SHIFT_AMOUNT);
  //   }
  //   //RSHFL
  //   else{
  //     SHIFTER_OUT = Low16bits(PS.AGEX_SR1 >> SHIFT_AMOUNT);
  //   }
  // }
  // else{
  //   //LSHF
  //   SHIFTER_OUT = PS.AGEX_SR1 << SHIFT_AMOUNT;
  // }

  if (((PS.AGEX_IR & 0x0030) >> 4) == 0 || ((PS.AGEX_IR & 0x0030) >> 4) == 2) { // LSHF 
    SHIFTER_OUT = PS.AGEX_SR1 << (PS.AGEX_IR & 0x000F); 
  }
  else if (((PS.AGEX_IR & 0x0030) >> 4) == 1) { // RSHFL 
    SHIFTER_OUT = PS.AGEX_SR1 >> (PS.AGEX_IR & 0x000F); 
  } 
  else if (((PS.AGEX_IR & 0x0030) >> 4) == 3) { // RSHFA 
    SHIFTER_OUT = signext(PS.AGEX_SR1, 15) >> (PS.AGEX_IR & 0x000F); 
  }

  //ALU
  int ALU_OUT;
  int SR2 = (Get_SR2MUX(PS.AGEX_CS)) ? signext(PS.AGEX_IR & 0x001F, 4) : PS.AGEX_SR2;
  ...
  // }
  // else {
  //   ALU_OUT = Low16bits(SR2);
  // }
  switch(Get_ALUK(PS.AGEX_CS)) {
        case(0):
            ALU_OUT = PS.AGEX_SR1 + SR2;
            break;
        case(1):
            ALU_OUT = PS.AGEX_SR1 & SR2;
            break;
  ...
  int math_result = Get_ALU_RESULTMUX(PS.AGEX_CS) ? ALU_OUT : SHIFTER_OUT;
  ...
  if (LD_MEM) {
    ...
    NEW_PS.MEM_ADDRESS = Low16bits(ADDRESSMUX_OUT);
    NEW_PS.MEM_NPC = PS.AGEX_NPC;
    NEW_PS.MEM_CC = PS.AGEX_CC;
    NEW_PS.MEM_ALU_RESULT = Low16bits(math_result);
...
void DE_stage() {
  ...
  //SR2.IDMUX, use bit 13 from IR
  // int SR2;
  // if (PS.DE_IR & 0x2000){
  //   SR2 = (PS.DE_IR>>9) & 0x0007;
  // }
  // else {
  //   SR2 = (PS.DE_IR & 0x0007);
  // }
  int SR2 = ((PS.DE_IR & 0x2000) >> 13) ? (PS.DE_IR & 0x0E00) >> 9 : PS.DE_IR & 0x0007; 
  int DR = Get_DRMUX(CONTROL_STORE[CONTROL_STORE_ADDRESS]) ? 7 : (PS.DE_IR & 0x0E00) >> 9;
  //
  dep_stall = 0;
  if (PS.DE_V){
    // variables for SR are lowercase !!!!!!!!
    if (Get_SR1_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS])){
      ...
      if(((SR1 == PS.AGEX_DRID) && V_AGEX_LD_REG) || ((SR1 == PS.MEM_DRID) && V_MEM_LD_REG) || ((SR1 == PS.SR_DRID) && v_sr_ld_reg)){
        dep_stall = 1; //set dependency stall if AGEX, MEM, or SR is modifying SR1
      }
    }
    // set for SR2
  ...
  // if none, then dep_stall will stay 0
  }
  ...
  if (LD_AGEX) {
    ...
    // NEW_PS.AGEX_DRID = Get_DRMUX(CONTROL_STORE[CONTROL_STORE_ADDRESS]) ? 7 : (PS.DE_IR & 0x0E00) >> 9;
    NEW_PS.AGEX_DRID = DR & 0x0007;
    NEW_PS.AGEX_V = PS.DE_V && !dep_stall && !mem_stall;
...
void FETCH_stage() {
  ...
  //insturction fetching from the cache
  icache_access(PC, &instruction_data, &icache_r);

  //stall logics for LD_DE
  ...
  //stall logic for LD_PC
  if(dep_stall || mem_stall || v_de_br_stall || v_agex_br_stall){
    LD_PC = 0; // if there is any stall, create a bubble
  } 
  else if (!icache_r && !v_mem_br_stall){
    LD_PC = 0; // if icache is not ready and there is no valid control instruction in the mem stage
  }
  ...
  //latch value to DE state

  if (LD_DE){
    NEW_PS.DE_NPC = Low16bits(PC + 2);
    NEW_PS.DE_IR = Low16bits(instruction_data);
    NEW_PS.DE_V = (icache_r) && (!v_mem_br_stall) && (!v_agex_br_stall) && (!v_de_br_stall);
  }

  // pc logic : dont move this around, remember Pc is needed for LD_DE !!!!! 
